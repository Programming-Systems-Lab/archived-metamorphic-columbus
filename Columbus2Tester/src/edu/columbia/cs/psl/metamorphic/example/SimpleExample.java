...
public class SimpleExample implements Cloneable {
	public SimpleExample() {
		...
		sort(sort(new String[] {"a","ddd","aaaa","zzza","a"}));
	}

	/*
	 * @
	 * 
	 * @meta findClosestValue(\multiply(values, 10), target * 10) == \result *
	 * 10
	 * 
	 ...
	 */
	@Metamorphic(rules = { @Rule(test = "findClosestValue(\\MultiplyByNumericConstant(values, 10), target * 10)", check = "\\result * 10"),
	...
	public int findClosestValue(int[] values, int target) {
		...
		int closestIndex = -1; // the index of the element that is closest to
								// the target
		for (int i = 0; i < values.length; i++) {
			// check the difference between the values and compare it to the
			// distance
	...
	@Metamorphic(rules = { @Rule(test="sort(\\Shuffle(in))", check = "\\result")})
	public ArrayList<String> sort(String[] in)
	{
	...
	@Metamorphic(rules = { @Rule(test="sort(\\Shuffle(in))", check = "\\result")})
	public ArrayList<String> sort(Collection<String> in)
	{
		ArrayList<String> ret = new ArrayList<String>();
		for(String s : in)
			ret.add(s);
	...
	public static void main(String[] args) {
		...
		// System.out.println(SimpleExample.findClosestValue(new int[]
		// {1,2,3,4,1000,10000,30000}, 300));
